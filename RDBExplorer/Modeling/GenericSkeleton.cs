...
namespace Metanoia.Modeling
{
    ...
    public class GenericSkeleton
    {
        ...
        public void RecomputeWorld()
        {
            for (int i = 0; i < Bones.Count; i++)
            {
                var bone = Bones[i];

                Matrix4 localMatrix = Matrix4.CreateScale(bone.Scale) *
                ...
                {
                    bone.WorldMatrix = localMatrix * Bones[bone.ParentIndex].WorldMatrix;
                }
                else
                {
    ...
    public class GenericBone
    {
        ...
        public static Quaternion ToEulerAngles(Matrix4 mat)
        {
            float x, y, z;
